      ...
      context 'A new Writer' do
        ...
        it 'should not be valid without a password' do
          ...
          it 'should not be valid with a confirmation mismatch' do
            @writer.attributes = { password: 'short',
                                password_confirmation: 'long'}
            @writer.should_not be_valid
        end
    
        it 'is not valid with a too long password' do
            @writer.password = '11111111111111111111111111111111111111111111111'
            expect(@writer).to_not be_valid
        end
        ...
        it 'is not valid if the email is not unique' do 
            @writer.save
            writer2 = Writer.new(name: 'test', email: 'tEsT@lL.Com', 
                                    password: '12345678')
            expect(writer2).to_not be_valid
        end

        it 'is not an admin by default ' do
            expect(@writer.admin?).to be(false) 
        end
